...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    th, td {
      padding: 15px 20px; text-align: left; border-bottom: 1px solid #f0f0f0;
    }
    ...
    .view-json-btn {
      padding: 4px 12px; font-size: 12px; background: #667eea;
      color: #fff; border: none; border-radius: 6px; cursor: pointer;
    }
    .tag {
      display: inline-block; padding: 2px 8px; background: #e6f7ff;
      color: #1890ff; border-radius: 4px; font-size: 11px; font-family: monospace;
    }
  </style>
</head>
<body>
  ...
  <div class="container">
    <div class="toolbar">
      ...
      <input type="text" id="search" placeholder="搜索名称/编码..." style="flex:1;padding:10px 15px;border:1px solid #ddd;border-radius:8px;" oninput="filterData()">
    </div>
    <div class="table-container">
      <table>
        <thead>
          <tr>
            <th>ID</th><th>名称</th><th>编码</th><th>父级</th><th>状态</th><th>创建人</th><th>Content</th><th>操作</th>
          </tr>
        </thead>
        <tbody id="table-body"><tr><td colspan="8" class="loading">加载中...</td></tr></tbody>
      </table>
    </div>
  </div>
    ...
    <div class="modal">
      ...
      <form id="json-form" onsubmit="handleSubmit(event)">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label>编码</label>
            <input type="text" id="code" placeholder="如：home-config">
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label>父级名称</label>
            <input type="text" id="parent_name" placeholder="可选">
          </div>
          <div class="form-group">
            <label>父级编码</label>
  ...
  <script>
    ...
    async function loadData() {
      try {
        ...
        else { document.getElementById('table-body').innerHTML = '<tr><td colspan="8" class="empty">加载失败：' + data.error + '</td></tr>'; }
      } catch (error) {
        document.getElementById('table-body').innerHTML = '<tr><td colspan="8" class="empty">加载失败：' + error.message + '</td></tr>';
      }
    }
    function renderTable(data) {
      const tbody = document.getElementById('table-body');
      if (data.length === 0) { tbody.innerHTML = '<tr><td colspan="8" class="empty">暂无数据</td></tr>'; return; }
      tbody.innerHTML = data.map(item => {
        let preview = '无内容';
        if (item.content) {
          try { const json = JSON.parse(item.content); preview = JSON.stringify(json).substring(0, 50) + '...'; }
          catch (e) { preview = item.content.substring(0, 50) + '...'; }
        }
        const parentInfo = item.parent_name ? \`<span class="tag">\${item.parent_name}</span>\` : '-';
        return \`<tr>
          <td>\${item.id}</td>
          <td>\${item.name}</td>
          <td>\${item.code ? '<span class="tag">'+item.code+'</span>' : '-'}</td>
          <td>\${parentInfo}</td>
          <td><span class="status-badge status-\${item.status}">\${item.status === 0 ? '禁用' : item.status === 1 ? '启用' : '归档'}</span></td>
          <td>\${item.create_user || '-'}</td>
          <td><button class="view-json-btn" onclick="viewJson('\${(item.content || '').replace(/'/g, "\\'")}')">👁 查看</button> <span style="margin-left:8px;color:#999;font-size:12px;">\${preview}</span></td>
          <td>
    ...
    function filterData() {
      ...
      if (keyword) { filtered = filtered.filter(item => item.name.toLowerCase().includes(keyword) || (item.code && item.code.toLowerCase().includes(keyword)) || (item.parent_name && item.parent_name.toLowerCase().includes(keyword)) || (item.parent_code && item.parent_code.toLowerCase().includes(keyword))); }
    ...
    function viewJson(content) {
      try { const json = JSON.parse(content); document.getElementById('json-content').textContent = JSON.stringify(json, null, 2); }
      catch (e) { document.getElementById('json-content').textContent = content || '无内容'; }
    ...
    async function handleSubmit(event) {
      ...
      if (content) { try { JSON.parse(content); } catch (e) { alert('JSON 格式无效：' + e.message); return; } }
      const data = { name: document.getElementById('name').value, code: document.getElementById('code').value || null, parent_name: document.getElementById('parent_name').value || null, parent_code: document.getElementById('parent_code').value || null, content: content || null, status: parseInt(document.getElementById('status').value), create_user: document.getElementById('create_user').value };
      try {
        const url = id ? \`/api/json-data/\${id}\` : '/api/json-data';
        const method = id ? 'PUT' : 'POST';
        const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
        const result = await res.json();
        if (result.success) { alert(id ? '更新成功' : '创建成功'); closeModal(); loadData(); }
    ...
    async function deleteItem(id) {
      ...
      try {
        ...
        const result = await res.json();
        if (result.success) { alert('删除成功'); loadData(); }

...
async function checkAuth() {
  try {
    ...
    if (data.success) {
      ...
      if (usernameEl) usernameEl.textContent = data.data.name || '管理员';
    } else {
...
async function loadStats() {
  try {
    ...
    if (users.success) {
      ...
      const el1 = document.getElementById('stat-users');
      const el2 = document.getElementById('stat-active');
    ...
    if (tokens.success) {
      ...
      const el1 = document.getElementById('stat-tokens');
      const el2 = document.getElementById('stat-tokens-active');
      if (el1) el1.textContent = total;
      if (el2) el2.textContent = active;
    }
    if (visits.success) {
      ...
      const el1 = document.getElementById('stat-visits');
      const el2 = document.getElementById('stat-avg');
      if (el1) el1.textContent = today;
      if (el2) el2.textContent = avg + 'ms';
    }
  } catch (e) {
...
if (typeof loadStats === 'function') loadStats();
  `.trim();
  return c.body(dashboardCode, 200, {
    'Content-Type': 'application/javascript',
    'Cache-Control': 'public, max-age=31536000'
  });
